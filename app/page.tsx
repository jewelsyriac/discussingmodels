  ...
  for (let part of parts) {
    if (part.match(sectionRegex)) {
      if (currentSubheading) {
        sections.push({ heading: currentSubheading, content: buffer.trim() });
        buffer = "";
      }
    ...
    <div>
      {sections.map((section, idx) => (
        <div key={idx} style={{ marginBottom: 18 }}>
          ...
          <div>
            {renderSectionWithLatex(section.content, idx)}
          </div>
        </div>
      ))}
    </div>
    ...
    <main>
      ...
      <form onSubmit={handleSubmit}>
        <textarea
          ...
          rows={4}
          style={{ width: "100%", fontSize: 16, overflow: 'hidden', resize: 'none' }}
        ...
        {result && (
          <button
            type="button"
            style={{ marginLeft: 12, background: '#f3f4f6', color: '#222', border: '1px solid #d1d5db', fontWeight: 500 }}
          ...
          {showBoth && result.answers && (
            <div className="model-compare-card" style={{ display: 'flex', gap: 16 }}>
              <div style={{ flex: 1, background: '#e0e7ff', border: '2px solid #6366f1', borderRadius: 10, padding: 12, position: 'relative' }}>
                ...
                {result.answers && result.answers[0]?.text && renderWithLatex(result.answers[0].text)}
                {result.answers && result.answers[0]?.text && (
                  <button
                    title="Copy Model 1"
              ...
              <div style={{ flex: 1, background: '#fef9c3', border: '2px solid #f59e42', borderRadius: 10, padding: 12, position: 'relative' }}>
                ...
                {result.answers && result.answers[1]?.text && renderWithLatex(result.answers[1].text)}
                {result.answers && result.answers[1]?.text && (
                  <button
                    title="Copy Model 2"
